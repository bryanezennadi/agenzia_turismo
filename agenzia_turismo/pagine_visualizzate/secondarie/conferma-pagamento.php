...
// Get event details from URL parameters or POST data
$evento_id = isset($_GET['evento_id']) ? intval($_GET['evento_id']) : (isset($_POST['evento_id']) ? intval($_POST['evento_id']) : 0);
$nome_evento = isset($_GET['nome_evento']) ? htmlspecialchars($_GET['nome_evento']) : (isset($_POST['nome_evento']) ? htmlspecialchars($_POST['nome_evento']) : 'Evento');
$num_biglietti = isset($_GET['num_biglietti']) ? intval($_GET['num_biglietti']) : (isset($_POST['num_biglietti']) ? intval($_POST['num_biglietti']) : 1);
$totale = isset($_GET['totale']) ? floatval($_GET['totale']) : (isset($_POST['totale']) ? floatval($_POST['totale']) : 0);
$data_evento = isset($_GET['data_evento']) ? htmlspecialchars($_GET['data_evento']) : (isset($_POST['data_evento']) ? htmlspecialchars($_POST['data_evento']) : 'Data non specificata');
$luogo = isset($_GET['luogo']) ? htmlspecialchars($_GET['luogo']) : (isset($_POST['luogo']) ? htmlspecialchars($_POST['luogo']) : 'Luogo non specificato');

// Generate order ID if not provided
$order_id = isset($_GET['order_id']) ? htmlspecialchars($_GET['order_id']) : 'ORD-' . rand(10000, 99999);
